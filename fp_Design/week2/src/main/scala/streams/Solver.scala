...
trait Solver extends GameDef {
  ...
  def from(
    ...
    if(initial.isEmpty) LazyList.empty
      ...
      initial.lazyAppendedAll(from(more,explored ++ more.map(_._1).toSet))
    }
  }
  ...
  /**
    ...
    */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] = neighborsWithHistory(Block(startPos,startPos),Nil)

  /**
    * Returns a lazy list of all possible pairs of the goal block along
    * with the history how it was reached.
    */
  lazy val pathsToGoal: LazyList[(Block, List[Move])] = from(pathsFromStart,Set.empty)
  ...
  lazy val solution: List[Move] = {
    val solutions = for{
      (block,history) <- pathsToGoal
      if(done(block))
    }yield(history)
    if(solutions.isEmpty) Nil else solutions.map(_.reverse).apply(0)
  }
}
