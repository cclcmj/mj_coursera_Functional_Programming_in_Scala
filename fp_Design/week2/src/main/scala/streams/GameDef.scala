...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    //   import ru.{MethodSymbol, MethodMirror}
    //   val m = ru.runtimeMirror(this.getClass.getClassLoader)
    //   val moveMethodSymb: (String) => MethodSymbol = (moveName) =>
    //     ru.typeOf[Block].member(ru.TermName(moveName)).asMethod
    //   val moveResult: (String) => MethodMirror = (moveName) =>
    //     m.reflect(this).reflectMethod(moveMethodSymb(moveName))
    //   List(Left, Right, Up, Down).map(mObj =>
    //     (
    //       moveResult(mObj.toString.toLowerCase)
    //         .apply() match {
    //           case b:Block => b
    //         },
    //       mObj
