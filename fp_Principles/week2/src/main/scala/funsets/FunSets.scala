...
trait FunSets extends FunSetsInterface {
   ...
   * that satisfies `p`.
   */
  def exists(s: FunSet, p: Int => Boolean): Boolean = forall(intersect(s,p),diff(s,s)) && forall(diff(s,s),intersect(s,p))

  /**
   * Returns a set transformed by applying `f` to each element of `s`.
   */
  def map(s: FunSet, f: Int => Int): FunSet = (x:Int)=>contains((a:Int)=>s(a)&&f(a)==x,x)
